...
def make_env(config: elements.Config, index: int, **overrides) -> embodied.Env:
    ...
    if suite == 'memmaze':
        ...
        ENVIRONMENTS["memmaze"] = lambda task, **kw: from_gym.FromGym(f'MemoryMaze-{task}-v0', **kw)
    environment = ENVIRONMENTS[suite]
    if isinstance(environment, str):
        module, cls = environment.split(':')
        module = importlib.import_module(module)
        environment = getattr(module, cls)
